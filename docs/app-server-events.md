...
The model-status payloads and terminal turn status are verified against the
experimental JSON schema generated by installed `codex-cli 0.155.0`:
`codex app-server generate-json-schema --experimental --out <temporary-directory>`.
The broader inventory above retains its upstream commit baseline; the sibling
Codex checkout is not present in this workspace.
...
Tool items are matched by item ID, so concurrent calls can finish in any order.
A late active update cannot overwrite a terminal result. Terminal turn status is
forwarded to the thread handler: failed turns mark unfinished tools failed, while
interrupted turns and tools without a reported result are marked interrupted.
A later authoritative tool result can still replace that provisional status.
Interrupted tools with no output are displayed as stopped, not still running.
`dynamicToolCall` items display the tool name, arguments, text results, and
reported outcome. Media results are represented by a concise notice; rich media
rendering and handling the `item/tool/call` execution request are not implemented.

## Model status notices

- `model/rerouted`
- `model/safetyBuffering/updated`
- `model/verification`

These supported notifications are normalized in `src/utils/appServerModelEvents.ts`
and displayed as informational conversation rows. Repeated updates replace the
same notice within a turn. Notices do not change the selected model, approvals,
or the thread's processing state, and do not resurrect hidden threads.
Verification notices report the server's values without assuming that verification
succeeded or that user action is required. Safety notices describe the most recent
server update; they are not pending tool executions.

These rows are derived from live notifications. Their restoration depends on the
local conversation cache; app-server thread snapshots do not contain these notices.
